...
def first_residual_block(input_layer, output_channel, bn_param, keep_prob, down_sample=False, device=None):
    ...
    with tf.variable_scope('layer1_in_block'):
        conv1 = bn_relu_conv(input_layer, [3, 3, input_channel, output_channel], strides=strides, bn_param=bn_param, device=device)
...
def residual_block(input_layer, output_channel, bn_param, keep_prob, device=None):
    ...
    with tf.variable_scope('layer1_in_block'):
        conv1= bn_relu_conv(input_layer, [3, 3, input_channel, output_channel], strides=[1,1,1,1],bn_param=bn_param, device=device)
    with tf.variable_scope('layer2_in_block'):
        conv2 = relu_dropout_conv(conv1, [3, 3, output_channel, output_channel], strides=[1,1,1,1], keep_prob=keep_prob, device=device)
...
# print(y)
# train_writer = tf.summary.FileWriter('/tmp/tmp1', graph=x)

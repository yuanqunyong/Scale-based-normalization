...
def relu_dropout_conv_bn(input_layer, filter_shape, strides, bn_param, keep_prob, device):
    ...
    layer.append(batch_norm(layer[-1], bn_param, device=device))
    return layer[-1]
...
def first_residual_block(input_layer, output_channel, bn_param, keep_prob, down_sample=False, device=None):
    ...
    with tf.variable_scope('layer2_in_block'):
        conv2 = relu_dropout_conv_bn(conv1, [3, 3, output_channel, output_channel], strides=[1,1,1,1], bn_param=bn_param, keep_prob=keep_prob, device=device)
...
print(y)
train_writer = tf.summary.FileWriter('/tmp/tmp1w36lq97', graph=x)

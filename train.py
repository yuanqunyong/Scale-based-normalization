            ...
            if FLAGS.task=='train_scaling':
              if '_c' in FLAGS.model :
                ...
                for i in range(2,5):
                  for k in range(n):
                    for var in weight:
                      if ('conv' in var.name) and ('group'+str(i)+'_block'+str(k) in var.name):
      ...
      for step in range(start_step, self.max_step):
        ...
        if (step + 1) % FLAGS.report_freq == 0:
          ...
          if FLAGS.verbose:
            if orthogonality_value:
              logging.info(
                'orthogonality(%d)=%f, %s' % (len(orthogonality_value), sum(orthogonality_value), orthogonality_value))
            if weightcost_value:
              logging.info('weightcost(%d)=%f, %s' % (len(weightcost_value), sum(weightcost_value), weightcost_value))
            logging.info('')
        ...
        if  (step + 1) % FLAGS.scal_freq== 0:
          if 'vgg' in FLAGS.model:
             ...
             # cross_entropy_value1,train_error1 = sess.run([cross_entropy,right], feed_dict=feed_dict)

             # if step>1:
             #   for k in range(totol_conv):
             #     print('now layer is %f ' % (k))
             #     if k < totol_conv - 1:
             #       var_value, mean_value = computer_var_mean(pre_activation[k])
             #       print(var_value)
             #       print(mean_value)
             #       conv_weight_1, conv_weight_2 = sess.run([var_con[k], var_con[k + 1]])
             #       scaling_value = np.abs(var_value / mean_value)
             #
             #       # scaling_value=np.mean(scaling_value,axis=(0,1))
             #       print(scaling_value)
             #       conv1, conv2 = gutils.conv_scal_select(conv_weight_1, conv_weight_2, scaling_value)
             #       var_con[k].load(conv1, sess)
             #       var_con[k + 1].load(conv2, sess)
             #     else:
             #       var_value, mean_value = computer_var_mean(pre_activation[k])
             #       print(var_value)
             #       print(mean_value)
             #       conv_weight_1 = sess.run(var_con[k])
             #       scaling_value = np.abs(var_value / mean_value)
             #       # scaling_value = np.mean(scaling_value, axis=(0, 1))
             #       print(scaling_value)
             #       conv1 = gutils.last_conv_scal_select(conv_weight_1, scaling_value)
             #       var_con[k].load(conv1, sess)



             # if (step+1) % 200==0:
             #   layer_weight_grad = sess.run(tf.gradients(losses, var_con[:totol_conv]),feed_dict=feed_dict)
             ...
             # cross_entropy_value2, train_error2 = sess.run([cross_entropy, right], feed_dict=feed_dict)
                 # layer_weight_grad = sess.run(tf.gradients(cross_entropy, var_con[:totol_conv]), feed_dict=feed_dict)
                 # layer_weight = sess.run(var_con[:totol_conv])
                 # layer_weight_adjust = backward_scaling_adjust_grad_mean(layer_weight_grad, layer_weight)
             ...
             #      var_con[k].load(conv1, sess)
             #
             # if (step > self.decay_step[0]) and (step <=self.decay_step[1]):
             #     for k in range(depth[0], totol_conv):
             #       if k < totol_conv - 1:
             #         var_value, mean_value = computer_var_mean(pre_activation[k])
             #         print(var_value)
             ...
             #         var_con[k].load(conv1, sess)
             #
             # if step > (self.decay_step[1]):#and (step <= self.decay_step[2]):
             #     for k in range(depth[0]+depth[1],totol_conv):
             #       if k < totol_conv - 1:
             #         var_value, mean_value = computer_var_mean(pre_activation[k])
             #         print(var_value)
             ...
             #
             #
             #   logging.info('variance_value= %s meanvalue= %s ' % (str(pre_activation_variance), str(pre_activation_mean)))
             # # for item in var_con:
               # print('before weight adjust  ________________________________')
               # tempp=np.abs(sess.run(item)[0])
               # logging.info('maxvalue= %f minvalue= %f meanvalue= %f '%(np.max(tempp),np.min(tempp),np.mean(tempp)))
             # print('between')
             # totol_conv = sum(depth)
             ...
             # for item in var_con:
             #   print('weight adjust  ________________________________')
             #   print(sess.run(item))
